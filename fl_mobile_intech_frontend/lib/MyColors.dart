import 'dart:ui';

class MyColors {

  static const Color COLOR_PRIMARY_ACCENT = Color(0xff016DD1);
  static const Color COLOR_APP_PRIMARY = Color(0xff2C98F0);
  static const Color COLOR_PRIMARY_BLUE = Color(0xffC1E0FF);
  static const Color BUTTON_ENABLED = Color(0xff182176);
  static const Color BUTTON_DISABLED = Color(0xffB5B5B5);
  static const Color GRADIENT_BLUE = Color(0xffC1E0FF);
  static const Color GRADIENT_WHITE = Color(0xffFFFFFF);
  static const Color OTP_BOXES = Color(0xffFFFFFF);
  static const Color RADIO_BUTTON = Color(0xff2E9F76);
  static const Color TEXT_COLOR = Color(0xff113E74);
  static const Color CIRCULAR_INDICATOR = Color(0xffDADADA);
  static const Color BLACK_TEXT = Color(0xff000000);
  static const Color OTP_BOX_OUTLINE = Color(0xff4285F4);
  static const Color OTP_BOX_INSIDE = Color(0xffBBBBBB);

}
